...
fn keyword_hover(word: &str) -> Option<String> {
    let info = match word {
        ...
        "tech-stack" => "Technology inventory organized by category.",
        "data-model" => "Data entities, fields, and entity relationships.",
        "trust-boundaries" => "Security zones grouping containers by trust level.",
        "teams" => "Team definitions with ownership assignments.",
        "views" => {
            "Diagram definitions: system-context-view, container, pipeline-view, deployment-view, etc."
        }
