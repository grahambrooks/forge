...
use crate::model::{ElementKind, Model, View, ViewKind};
use crate::text::*;
...
pub fn render_view(model: &Model, view: &View, style: &str) -> String {
    if view.kind == ViewKind::Composite {
        return render_composite(model, view, style);
    }
    let lo = crate::layout::compute_layout(model, view);
    render_svg(&lo, style)
}

fn render_composite(model: &Model, view: &View, style: &str) -> String {
...
"#;
        ...
        let comp_view = model
            ...
            .unwrap();
        let svg = super::render_view(&model, comp_view, "filled");

        // Title rendered
        assert!(svg.contains("Exec Dashboard"));
        // CSS class on root marks it as composite
        assert!(svg.contains("forge-composite"));
        // Two child <svg> cells with data-view attributes for each referenced view
        assert!(svg.contains(r#"data-view="Context""#));
        assert!(svg.contains(r#"data-view="Containers""#));
        // Each cell gets a frame rect
...
"#;
        ...
        let svg = super::render_view(&model, outer, "filled");
        // Outer renders but the Inner cell is skipped (no data-view for Inner
        // since we short-circuit on nested composites).
        assert!(svg.contains("forge-composite"));
...
"#;
        ...
        // Step badge circles and labels present
        assert!(
        ...
        // All three numbers rendered (check for the text content)
        for n in ["1", "2", "3"] {
            assert!(
